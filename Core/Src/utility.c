...
uint8_t us_num=0; //达到1us所需要的时钟震荡次数

//填写自己芯片最大的时钟频率。切记 使用了自定义的延迟函数后就不能使用
...
void delay_init(uint16_t sysclk)
{
    ...
    // 2. 配置预分频器，使计数器频率为1MHz(1us计数一次)
    DELAY_TIMx->PSC = sysclk - 1;

    // 3. 向上计数 + 单脉冲模式
    DELAY_TIMx->CR1 &= ~TIM_CR1_DIR;
    DELAY_TIMx->CR1 |= TIM_CR1_OPM;
...
void delay_us(uint16_t time)
{
    ...
    // 3. 强制复位 TIM 状态(防止外设异常时 OPM 锁定导致卡死)
    DELAY_TIMx->CR1 = 0;        // 清零 CR1 (包括 CEN 和 OPM)
    DELAY_TIMx->EGR = TIM_EGR_UG; // 产生更新事件,重新初始化
    DELAY_TIMx->SR = 0;          // 再清一次状态
    DELAY_TIMx->CNT = 0;         // 清计数器

    // 4. 重新配置单次模式并启动
    DELAY_TIMx->CR1 |= TIM_CR1_OPM | TIM_CR1_CEN;

    // 5. 等待计数器停止,带超时保护(防止极端情况下死循环)
    uint32_t timeout = 0xFFFFFF;
    while((DELAY_TIMx->CR1 & TIM_CR1_CEN) != 0 && --timeout);
    if(timeout==0)
    {
        DELAY_TIMx->CR1 = 0;
        DELAY_TIMx->SR = 0;
...
/// @brief 设置中断向量表
/// @param baseAddr 程序起始地址
/// @param offset 偏移的地址。偏移地址必须是0x200的倍数，即512B的倍数
...
uint8_t SYS_JumpApp(uint32_t appBaseAddr)
{
    ...
    {
        if (((*(volatile  uint32_t *)appBaseAddr) & 0x2FFE0000) == 0x20000000)     /* 检查栈顶地址是否合法 */
        {
...
/// @brief 软件复位(OTA 升级完成后切换固件使用)
void Sys_SoftReset()
{
    //关闭所有中断， 只屏蔽普通中断，不屏蔽HardFault等异常和不可屏蔽中断
    __disable_irq(); 

    //屏蔽除 不可屏蔽中断以外的所有中断和异常
    __set_FAULTMASK(1);
...
{
    if(rb->cnt==rb->size) return UTIL_RES_ERR;
...
{
    if(rb->cnt==0) return UTIL_RES_ERR;
...
int RingBuffer_Init(RingBuffer_t* rb,uint16_t size)
{
    ...
    if(rb->data==NULL)
    {
        LOG_E(UTIL_RES_ERR,"pvPortMalloc");
        return UTIL_RES_ERR;
    }
...
{
    if(rb->data==NULL) return;

#ifdef USE_FreeRTOS
    if(rb->rwMutex!=NULL) xSemaphoreTake(rb->rwMutex,portMAX_DELAY);
#endif
...
#ifdef USE_FreeRTOS
    if(rb->rwMutex!=NULL) xSemaphoreGive(rb->rwMutex);
#endif
}
...
/*CRC16_Calculate;按字节计算CRC
  ...
  注意: 初始值为 0xFFFF, 计算过程未取反(与上位机保持一致即可)
*/
