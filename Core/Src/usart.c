...
{
  // 阻塞方式打印 -> 串口1
  UART_Transmit_String((uint8_t *)&ch, 1,2);
  return ch;
}
...
void HAL_UART_MspInit(UART_HandleTypeDef* huart)
{
  ...
  if(huart->Instance==USARTx2)
  {
    ...
    GPIO_InitStruct.Alternate=USARTx2_GPIO_AF;
...
/// @brief 获取接收数据的指针。使用该函数需要自己调用清除接收完成标志位。即调用 UART_ClearRxFlag(usartId);
/// @param rxData 接收数据的指针，也就是用于串口接收数据缓存区的指针
/// @param usartId 1,2
/// @return 返回0代表有数据接收，1代表没有
uint8_t UART_GetRxDataPointer(char** rxData,uint8_t usartId)
{
  ...
  {
    if(UART_GetRxFlag(1)==0) return 1; //没有数据接收
    *rxData=(char*)uart1_receiveBuf;
  }
  else if(usartId==2)
  {
    if(UART_GetRxFlag(2)==0) return 1; //没有数据接收
    *rxData=(char*)uart2_receiveBuf;
  ...
  else if(usartId==3)
  {
    ...
    // printf("p2:%d\n",rxData);
    // printf("u3:%d\n",uart3_receiveBuf);
    uint16_t rxSize=UART_GetRxDataSize(usartId);
    if(rxSize==UART3_RX_DATA_SIZE) uart3_receiveBuf[rxSize-1]='\0';
    else uart3_receiveBuf[rxSize]='\0';
    *rxData=(char*)uart3_receiveBuf;
    // printf("u4:%d\n",*rxData);
  ...
  {
    if(UART_GetRxFlag(4)==0) return 1; //没有数据接收
    *rxData=(char*)uart4_receiveBuf;
    
  }
  else if(usartId==5)
  {
    if(UART_GetRxFlag(5)==0) return 1; //没有数据接收
    *rxData=(char*)uart5_receiveBuf;
  }
  return 0;
}
...
/*获取串口缓存区指针*/
char* UART_GetRxData2(uint8_t usartId)
{
  if(usartId==3) return uart3_receiveBuf;
}
