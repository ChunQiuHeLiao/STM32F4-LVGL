...
void setup_scr_screen_album_show(lv_ui *ui)
{
    ...
    lv_obj_set_pos(ui->screen_album_show_btn_show_img_info, 200, 8);
    lv_obj_set_size(ui->screen_album_show_btn_show_img_info, 20, 12);
    ...
    lv_obj_set_style_bg_opa(ui->screen_album_show_cont_img_info, 27, LV_PART_MAIN|LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(ui->screen_album_show_cont_img_info, lv_color_hex(0x000000), LV_PART_MAIN|LV_STATE_DEFAULT);
...
static void Album_Show_EventHandler(lv_event_t* e)
{
    ...
    if (strcmp(str, "scroll") == 0)
    {
        ...
        {
            printf("left\n");
            if ((img_index+1) == albumHandle.num) return; /*达到最后一张*/
            img_index++;
        ...
        /*通过索引获取图片名*/
        char imgName[32] = { 0 };
        ML_GetFileName_By_Index(img_index, IMG_DIR,imgName,sizeof(imgName));
        lv_label_set_text(guider_ui.screen_album_show_label_img_name, imgName); /*修改图片标题*/
        printf("img_name:%s\n", imgName);

        char imgPath[48] = { 0 };
        sprintf(imgPath, "D:/%s", imgName);
        printf("s:%s\n", imgPath);
#if LV_USE_GUIDER_SIMULATOR==0
        LCD_DispImg((const char*)imgPath);
#endif
    }
}
...
void Screen_Album_Show_UI_Init(lv_ui* ui)
{
    ...
    img_index = albumHandle.index;
    //uint8_t waitTime = lv_timer_handler();

    //LCD_DispImg(img_path);
}
