...
{
    ST7789_MultiColor_Fill(x, y, x + width - 1, y + height - 1, color);     /* 填充 */
}
...
void piclib_init(void)
{
    ...
    picinfo.lcdwidth = ST7789_GetCurWidth();        /* 得到LCD的宽度像素 */
    picinfo.lcdheight = ST7789_GetCurHeight();      /* 得到LCD的高度像素 */
...
uint8_t piclib_ai_load_picfile(char *filename, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t fast)
{
    ...
    {
        picinfo.S_Height = picinfo.lcdheight;
        picinfo.S_Width = picinfo.lcdwidth;
        return FALSE;
    }
...
{
    //return (void *)memset(SRAMIN, size);
