...
static size_t jpeg_in_func(JDEC *jd, uint8_t *buf, size_t num)
{
    ...
    if (buf!=NULL)        /* 读取数据有效，开始读取数据 */
    {
        uint8_t ret=f_read(dev, buf, num, (UINT *)&rb); /* 调用FATFS的f_read函数，用于把jpeg文件的数据读取出来 */
        if(ret) printf("f_read fail:%d\n",ret);
        if(rb==0) printf("ret:%d,num:%d\n",rb,times);
        times+=rb;
